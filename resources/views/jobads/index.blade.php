...
@section('contentsearch')
        <form class="form-horizontal searchForm job" method="post" role="form" action="{{ url('jobs/search') }}">
			@csrf
 				@if (isset($candidate))
...
@include('partials.searchfilters.navbar_search_filter', ['filterName'=> 'q[jobref]',
		...
		<div id="jbdFilterBody" class="searchFilters searchForm job collapse @if($isExpanded) show @endif" aria-labelledby="jbdFilterHeading">
...
@include('partials.select2_filter_dropdown_single', ['fieldname'=>'q[workserviced]', 'fieldlabel'=>'Work Serviced', 
									'fieldplaceholder'=>'', 
									'options'=>[''=>'--','t'=>'Yes', 'f'=>'No'],
									'selectedoptions'=>old('q[workserviced]', isset($q['workserviced']) ? $q['workserviced'] : null)])
					</div>
...
@include('scripts.ready_select2_form_reset');		
	@include('scripts.ready_markjs', ['fields'=>"['cvsend', 'summary', 'agencynotes', 'projectplan', 'skills']"]);
