...
@if ($item->isFillable('colour_hex'))
                        ...
                        <div class="form-group row">
							...
							<legend>Highlight</legend>
								<input class="form-control-check" placeholder="" name="colour_no" type="checkbox" id="colour_no" {{ old('colour_hex', isset($item) ? $item->colour_hex : "" ) ? '' : 'checked' }}>
								<label for="colour_no" class="form-check-label mr-3">{{ __('No Highlight') }}</label>
								<input class="form-control-color btn-sm" title="Choose colour" placeholder="" name="colour_hex" default="#ffffff" type="{{ old('colour_hex', isset($item) ? $item->colour_hex : "" ) ? 'color' : 'hidden' }}" id="colour_hex" value="{{ old('colour_hex', isset($item) ? $item->colour_hex : "" ) }}">
							</fieldset>	
                        </div>								
