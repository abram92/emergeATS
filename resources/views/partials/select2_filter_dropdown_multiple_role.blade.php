		...
		<select @if(isset($fieldid)) id="{{ $fieldid }}" @endif class="@if(isset($select2_class)) {{ __($select2_class) }} @else select-select2-optional @endif" data-placeholder="{{ $fieldplaceholder }}" style="width:100%;" tabindex="4" multiple name="{{ $fieldname }}">
			@if(!empty($options))
				@foreach($options as $optionid => $optionvalue)
					...
					@else 
				       <option value="{{ $optionid }}" @if(isset($selectedoptions) && in_array($optionid, $selectedoptions)) selected="selected" @endif @if ($optionvalue) style="background-color:{{ $optionvalue }}"  data-color="{{ $optionvalue }}" @endif>{{ $optionid }}</option>
				    @endif				  
				@endforeach
            @endif
		</select>
