...
class import_static_work_alert_candidates extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'import:static_work_alert_candidates {filename} {deleteBefore=0} {delimiter=,}';
...
//		$classcolumn = $valid[$type]['fields'];
 ...
 //   $header = ['id', 'modelid', 'chunk'];
    $data = array();
		...
		if ($deleteBefore) {
			$sql = "delete from static_work_alert_candidates;"; // where auditable_type = ".$this->formatStr($valid[$type]['class']).";";
			DB::statement($sql);
		}
...
//			$rows[] = $cells;
		...
		$sql = "insert into static_work_alert_candidates(alert_id, candidate_id, alert_level, level_ordinal, status_id)
OVERRIDING SYSTEM VALUE VALUES ";

$sql .= "(".(int)$d['alertid'].",".(int)$d['candidateid'].",".(int)$d['alertlevel'].",".(int)$d['levelordinal'].",".(int)$d['statusid']." )";
	 echo $sql;
