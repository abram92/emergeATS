...
class import_static_work_emails extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'import:static_work_emails {filename} {deleteBefore=0} {delimiter=,}';
...
//		$classcolumn = $valid[$type]['fields'];
 ...
 //   $header = ['id', 'modelid', 'chunk'];
    $data = array();
		...
		if ($deleteBefore) {
			$sql = "delete from static_work_emails;"; // where auditable_type = ".$this->formatStr($valid[$type]['class']).";";
			DB::statement($sql);
		}
...
if (is_numeric($d['timestamp'])) {
	...
	$createdate = $d['timestamp'];
		...
		$sql = "insert into static_work_emails(id, consultant_id, created_at)
OVERRIDING SYSTEM VALUE VALUES ";

$sql .= "(".(int)$d['id'].",".(int)$d['consultantid'].",".$this->formatStr($createdate)." )";
$sql .= " ON CONFLICT (id) 
DO
      UPDATE
     SET consultant_id = EXCLUDED.consultant_id, created_at = EXCLUDED.created_at;";
	 echo $sql;
