...
class import_static_work_email_recipients extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'import:static_work_email_recipients {filename} {deleteBefore=0} {delimiter=,}';
...
/*		$valid = ['client'=>['class'=>'App\Client','fields'=>'clientid'],
		...
		'recipient'=>['class'=>'App\recipientAd','fields'=>'recipientid'],
		];
*/		
...
//		$classcolumn = $valid[$type]['fields'];
 ...
 //   $header = ['id', 'modelid', 'chunk'];
    $data = array();
		...
		if ($deleteBefore) {
			$sql = "delete from static_work_email_recipients;"; // where auditable_type = ".$this->formatStr($valid[$type]['class']).";";
			DB::statement($sql);
		}
...
//			$rows[] = $cells;
		...
		$sql = "insert into static_work_email_recipients(email_id, user_id)
OVERRIDING SYSTEM VALUE VALUES ";

$sql .= "(".(int)$d['emailid'].",".(int)$d['userid']." )";
	 echo $sql;
