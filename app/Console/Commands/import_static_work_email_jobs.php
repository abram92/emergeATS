...
class import_static_work_email_jobs extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'import:static_work_email_jobs {filename} {deleteBefore=0} {delimiter=,}';
...
//		$classcolumn = $valid[$type]['fields'];
 ...
 //   $header = ['id', 'modelid', 'chunk'];
    $data = array();
		...
		if ($deleteBefore) {
			$sql = "delete from static_work_email_job_ads;"; // where auditable_type = ".$this->formatStr($valid[$type]['class']).";";
			DB::statement($sql);
		}
...
$reminder = (int)$d['reminder'] ? "'t'" : "'f'";
		$sql = "insert into static_work_email_job_ads(email_id, job_ad_id, status_id, reminder)
OVERRIDING SYSTEM VALUE VALUES ";

$sql .= "(".(int)$d['emailid'].",".(int)$d['jobid'].",".(int)$d['statusid'].",".$reminder." )";
	 echo $sql;
