...
class import_cand_preferredlocations extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'import:cand_pref_loc {filename} {deleteBefore=0} {delimiter=,}';
 ...
 //   $header = ['id', 'modelid', 'chunk'];
    $data = array();
		...
		$rows = [];
		$excluded = []; //[81469, 30347];
		$success = false;
		DB::beginTransaction();
		try {

if ($deleteBefore) {
	$sql = "delete from candidate_preferred_location;";
	 DB::statement($sql);
}		
...
if($results[0]->count) { 
		$sql = "insert into candidate_preferred_location(candidate_id, location_id)
 VALUES ";

$sql .= "(".(int)$d['candid'].",".(int)$d['location']." );";
// $sql .= " ON CONFLICT (candidate_id, location_id) 
//DO
 //     UPDATE
 //    SET candidate_id = EXCLUDED.candidate_id, location_id = EXCLUDED.location_id;";
	 echo $sql;
	 
	 DB::statement($sql);
 } else {
	 $excluded[] = (int)$d['candid'];
 }
			}
		}
//	dd($rows);	
