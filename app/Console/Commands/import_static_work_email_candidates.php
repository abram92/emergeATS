...
class import_static_work_email_candidates extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'import:static_work_email_candidates {filename} {deleteBefore=0} {delimiter=,}';
...
/*		$valid = ['client'=>['class'=>'App\Client','fields'=>'clientid'],
		...
		'candidate'=>['class'=>'App\candidateAd','fields'=>'candidateid'],
		];
*/		
...
//		$classcolumn = $valid[$type]['fields'];
 ...
 //   $header = ['id', 'modelid', 'chunk'];
    $data = array();
		...
		if ($deleteBefore) {
			$sql = "delete from static_work_email_candidates;"; // where auditable_type = ".$this->formatStr($valid[$type]['class']).";";
			DB::statement($sql);
		}
...
//			$rows[] = $cells;
		...
		$sql = "insert into static_work_email_candidates(email_id, candidate_id, status_id, reminder)
OVERRIDING SYSTEM VALUE VALUES ";

$sql .= "(".(int)$d['emailid'].",".(int)$d['candidateid'].",".(int)$d['statusid'].",".(int)$d['reminder']." )";
	 echo $sql;
